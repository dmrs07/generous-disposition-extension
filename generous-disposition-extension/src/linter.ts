import { BlockName, CANONICAL_BLOCKS, GDDocument } from './ast';

export interface GDDiagnostic {
...
function computeComplexityScore(doc: GDDocument): number {
  let score = 0;
...
export function lintGD(doc: GDDocument): GDDiagnostic[] {
  const diagnostics: GDDiagnostic[] = [];
  ...
  const canonicalOrder = CANONICAL_BLOCKS.filter(n => presentBlocks.includes(n));
